...
public class TreeVisualizer : MonoBehaviour
{
    ...
    public TreeType treeType = TreeType.BST;
        ...
        where TKey : IComparable<TKey>
    {
        ...
        switch (spacingType)
        {
            case SpacingType.Pow:
                AssignPositionsPow(root, Vector3.zero, root.Height, root.Height);//
                break;
        ...
        where TKey : IComparable<TKey>
    {
        ...
        float offset = Mathf.Pow(2, height - 1) * (horizontalSpacing / totalHeight);
        Vector3 childBase = position + Vector3.down * verticalSpacing;

        AssignPositionsPow(node.Left, childBase + Vector3.left * offset, height - 1, totalHeight);
        AssignPositionsPow(node.Right, childBase + Vector3.right * offset, height - 1, totalHeight);
    }
        ...
        where TKey : IComparable<TKey>
    {
        ...
        nodePositions[node.Key] = new Vector3(xIndex * horizontalSpacing, -depth * verticalSpacing, 0);
        xIndex++;
        ...
        where TKey : IComparable<TKey>
    {
        ...
        for (int depth = 0; depth < levels.Count; depth++)
        {
            ...
            {
                nodePositions[row[i].Key] = new Vector3(i * horizontalSpacing, yPos, 0f);
            }
        }
    }
